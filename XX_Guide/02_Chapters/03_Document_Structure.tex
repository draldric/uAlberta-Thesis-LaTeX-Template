...
\chapter{Document Structure and Formatting}\label{ch:documentstructure}
	...
	\section{Title Page, Abstract, and Other Prefatory Pages}
		...
		However, for a thesis at the \University, a few more pieces of information are required for the title page, including:
		\begin{itemize}
			\item Degree,
			\item Department or Faculty,
			\item Specialization, and 
			\item Creative Commons.
		\end{itemize}
		To help with this I have recreated the title page to match the sample provided by the \Faculty\ and have add corresponding commands to set the content for these.
		Additionally, a few more pages are also required (Abstract and Preface) and a few optional pages may be included as well (Quote, Dedication, and Acknowledgements).
		All of these pages have very specific formatting requirements provided by the \Fac.
		
		The \Faculty\ even provides a few different documents that provide the necessary information to create these pages \cite{FGPS2024}.
		While these do a decent job at describing how these pages need to be laid out, I decided to take the guess work completely out of it. 
		...
		These commands look for their respective \texttt{*.tex}\footnote{If you would rather set the text using a macro to keep the text in the same file, change \texttt{get} in the respective command to \texttt{set} and write in the content. \textit{e.g.} \cmd{setabstracttext}\mopt{<Your Content Here>}} file in the \path{01_LaTeX_Prefatory} folder.
		These commands can be commented out if they are unused and their respective sections will be excluded completely from the document or in some cases set to a predetermined output.
		\begin{itemize}
			\item \cmd{getabstracttext}\mopt{}, 
			\item \cmd{getprefacetext}\mopt{}, 
			\item \cmd{getquotetext}\mopt{}, 
			\item \cmd{getdedicationtext}\mopt{}, and 
			\item \cmd{getacknowledgementtext}\mopt{}
		\end{itemize}
		...
		It will analyze if the commands that are used to set the text are empty or commented out and automatically exclude those sections or in the case of the preface it will default to the minimum required preface example, see \Cref{preface:DNA}.
		...
		\subsection{Abstract}\label{abstract}
			...
			To edit the abstract open the file \path{01_Prefatory/Abstract.tex} and add your text to the file.
		...
		\subsection{Preface}\label{preface}
			...
			To edit the preface open the file \path{01_Prefatory/Preface.tex} and add your text to the file.
			If file is left blank then the default mandatory preface will be used.
		...
		\subsection{Dedication or Quotations}\label{quote}\label{dedication}
			...
			In the template provided, you may fill in the appropriate \path{Quote} and \path{Dedication} files from the prefatory folder: \path{01_Prefatory/}.
			If they are left blank, then they will automatically be excluded.
			Or you can just comment out the command \cmd{getquotetext} or \cmd{getdedicationtext} if you are unsure of whether you want to include one now or not and would just like to temporarily disable one or both of them.
		...
		\subsection{Acknowledgements}\label{acknowledgement}
			The template automatically includes and formats your acknowledgements according to the \Faculty's requirements, using the \cmd{makeacknowledgements} command.
			To edit the acknowledgements open the file \path{01_Prefatory/Acknowledgements.tex} and add your text to the file.
		...
		\subsection{Table of Contents}\label{toc}
			For your thesis, this template will automatically include all required chapters including the correct ones from the prefatory pages.
		...
		\subsection{Lists of Figures, Tables, ...}\label{listsof}
			This template is setup to for figures, tables, plates, illustrations, photos, videos, and sound files.
			Each of items has their own environment \env{figure}, \env{table}, \env{plate}, \env{illustration}, \env{photo}, \env{video}, and \env{sound}, respectively.
			These following set of smart-commands will automatically include or exclude the respective \enquote{List of ...} in the prefatory pages, no need to comment or uncomment them.
			If you would like them to appear in a different order, just change their order in the the \path{ualberta.tex} file.
			\begin{itemize}
				\item \cmd{listoffigures}
				\item \cmd{listoftables}
				\item \cmd{listofplates}
				\item \cmd{listofillustrations}
				\item \cmd{listofvideos}
				\item \cmd{listofphotos}
				\item \cmd{listofsounds}
			\end{itemize}
	...
	\section{Nomenclature, Glossary, \& Acronyms}\label{nomenclature}\label{glossary}\label{acronyms}
		\subsection{Lists of Symbols}
			The \cmd{listofsymbols} command will print the List of Symbols.
			\begin{center}
				\cmd{nomenclature}\oopt{CATEGORY}\mopt{SYMBOL}\mopt{DEFINITION}
			\end{center}

			Adding the \cmd{nomunit} command inside and at the end of the \opt{DEFINITION} will add its \opt{CONTENTS} to the end of the line with the content being right justified.
			\cmd{nomunit}\mopt{CONTENTS}

			To add more categories to sort the Nomenclature or List of Symbols by you just need to add a new line to the code, show in \Cref{lst:nomencalturesettings}, at the top of the \path{Nomencalture.tex} file in \path{01_Prefatory} folder.
			
			\begin{lstlisting}[caption=Nomenclature Grouping Settings,label=lst:nomencalturesettings]
\renewcommand\nomgroup[1]{%
	\item[\bfseries%
		...
		\subsection{Lists of Abbreviations}
			The \cmd{abbreviations} command will print the List of Symbols.
			\begin{center}
				\cmd{addacronym}\mopt{ACRONYM}\mopt{DEFINITION}
			\end{center}

		\subsection{Glossary of Terms}
			The \cmd{generateglossary} command will print the List of Symbols.
			\begin{center}
				\cmd{addterm}\mopt{TERM}\mopt{DEFINITION}
			\end{center}
	...
	\section{Chapters, Sections, Subsections, \textit{etc.}}
		...
		These structures all utilize the base macros from \LaTeX\ including:
			\begin{itemize}
				...
				\item \cmd{paragraph}\mopt{Paragraph Heading}, and
