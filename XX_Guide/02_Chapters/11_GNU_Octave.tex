...
\chaptermark{GNU Octave}
	...
	\section{Octave in a \LaTeX\ Thesis Workflow}
		...
		\subsection{Exporting Vector Graphics}
			...
			\end{center}
			This file can then be directly included in your thesis using the standard \cmd{includegraphics} command or the custom command \cmd{insertimage}\footnote{The \cmd{insertimage} just wraps the \cmd{includegraphics} command by automatically adding the image directory to the filename. \par\textit{i.e.}, \cmd{insertimage}\mopt{filename} is equivalent to \cmd{includegraphics}\mopt{./99_Inclusions/Images/filename}.} within a \env{figure} environment.
