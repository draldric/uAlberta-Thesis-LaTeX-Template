...
\chapter{Plots, Charts, \& Graphs}\label{ch:plotsandgraphs}
		...
		Mainly this will be accomplished with the \pkg{pgfplots}\footfullcite{pgfplots} package.
	...
	\section{Line Plots}
		...
		\begin{figure}[p]
			\centering
			\LTXinputExample[pos=t,preset=\centering]{\insertplotcode{simpleLinePlot}}
			\caption[A simple line plot.]{A simple line plot and the code to generate the plot }\label{fig:line-plot}
		\end{figure}


		Expanding on this example, we can add a second plot by adding the code shown below \Cref{fig:line-plot-2}.
		This will result in the addition of the second red line.
		\begin{figure}[p]
			\centering
			\LTXinputExample[pos=t,preset=\centering]{\insertplotcode{doubleLinePlot}}
	...
	\section{Customizing Plots}
		...
		\subsection{Increasing the Plot Size}
		...
		\subsection{Adding a Legend}
			...
			\note{
				The position of the legend can be specified by using the optional parameter \opt{legend pos=} followed by a set of compass coordinates.}
