\chapter{Example Chapter}\label{ch:ExampleChapter}
	This chapter aims to provide examples of how to structure and create specific components in your thesis document.
	Throughout this section, \LaTeX's automatic placement for figures and tables will be disabled in most cases.
	This is being done to add a specific flow to \textbf{THIS DOCUMENT}; this should be avoided in one's own thesis as it can lead to poor placement of figures, tables, and other \enquote{floats}, as well as cause unnecessary white space.
	\section{General Text Layouts}
		\subsection{Document Structure}
			\LaTeX{} provides a set of commands that can be used to provide the structure for your document.
			Depending on the document class, the different commands shown in \Cref{tab:docstructure} may be used.
			Originally this thesis template was created as a modification to the \texttt{book} class, however, it included certain definitions that are unlikely to be included in a thesis document.
			Due to this, the latest versions of this document (2.0.0+) \textbf{do not} include definitions for \texttt{part}.
			
			\begin{table}[H]
				\caption{Document Structure Commands}
				\label{tab:docstructure}
				\centering
				\begin{tabularx}{0.85\textwidth}{ccC} 
					\toprule
						\textbf{Level} & \textbf{Command} & \textbf{Notes}\\
					\midrule
						-1 & \lstinline{\\part\{\}} & Not Recommended for Thesis\\
						 0 & \lstinline{\\chapter\{\}}\\
						 1 & \lstinline{\\section\{\}}\\
						 2 & \lstinline{\\subsection\{\}}\\
						 3 & \lstinline{\\subsubsection\{\}}\\
						 4 & \lstinline{\\paragraph\{\}}\\
						 5 & \lstinline{\\subparagraph\{\}}& Not Recommended for Thesis\\
					\bottomrule
				\end{tabularx}
			\end{table}
	...
	\section{Tables}
		...
		On the other hand, the \path{tabularx} package allows us to set the width of the table and inform it which columns should be scaled to fit the data and the \path{tabularx} package will handle the heavy lifting for us.
		...
		This column type, or one of the derivatived column types shown below, must be included in the use of a \path{tabularx} table. 
		Failing to do so will cause an error. The additional column types that one might like to create for use with the \path{tabularx} environment are as follows (Note that the \path{X} column type is a Fully Justified Auto Sizing Column):
		\begin{itemize}
		...
		\end{itemize}
  		These new column types should be included in the preamble (before the \lstinline|\begin{document}|; better yet, to help keep things organized it is recommended to include them in the \verb|includeMacros.tex| file.
		\begin{table}[H]
		...
		Beyond this, to include horizontal lines, the \lstinline|\cmidrule(l{<length>}r{<length>}){start_cell-end_cell}| from the \path{booktabs} package is used.
		To add a bit of a cleaner look the lines can be trimmed by including the \path{l{<length>}} or \path{r{<length>}} in the \textbf{round} brackets to trim the lines by the specified length for the left and right sides, respectively.
	...
	\section{Figures}
		...
		\begin{figure}[H]
			...
			\begin{subfigure}{0.45\textwidth}
				...
				\caption{} % Leave blank for just the letter to show up.
	...
	\section{Graphs \& Plots}
		...
		Though this can be quite useful, I will be excluding indepth information on creating these.
