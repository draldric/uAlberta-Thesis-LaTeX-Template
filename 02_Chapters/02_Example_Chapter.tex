\chapter{Example Chapter}\label{ch:ExampleChapter}
  ...
  \section{General Text Layouts}
    \subsection{Lists}
        ...
        Descriptions should have the optional argument included for each \path{item} otherwise the line will have a blank in the beginning like the \textit{Second Item} in the next list and not align with the other items.
        \begin{description}
    ...
    \subsection{Footnotes}
        ...
        \path{\footnote{This is where the foot note text goes.}}\footnote{This is where the footnote text goes for the in-line footnote.} 
        ...
        And the macro \path{\footnotetext[n]{This is where the footnote text goes for a split footnote.}}.
        This allow you to help keep your document more organized with the drawback that it is the user's responsibility to keep track of the foot note numbers or ensure that a \path{\footnotemark{}} is followed by it's corresponding \path{\footnotetext{<text>}}.
        For this reason, I recommend using the in-line footnotes.
    
        \footnotetext{This is where the footnote text goes for a split footnote.}
  ...
  \subsection{Cross-References}\label{subsec:cross-reference}
      ...
      \path{\section{Citations and References}\label{sec:citref}}
      ...
      \begin{table}[H]
        ...
        \begin{tabularx}{0.5\textwidth}{CC} 
          ...
          \midrule
            ...
            \path{\cref*{}}         & \cref*{tab:reftable} \\
            %\path{\Cref*{}}         & \Cref*{tab:reftable} \\
            \path{\cpageref{}}      & \cpageref{tab:reftable} \\
            \path{\Cpageref{}}      & \Cpageref{tab:reftable} \\
            \path{\namecref{}}      & \namecref{tab:reftable} \\
            \path{\nameCref{}}      & \nameCref{tab:reftable} \\
            %\path{\lcnamecref{}}    & \lcnamecref{tab:reftable} \\
            %\path{\labelcref{}}     & \labelcref{tab:reftable} \\
            %\path{\labelcpageref{}} & \labelcpageref{tab:reftable} \\
          \bottomrule
        \end{tabularx}
        \end{table}
  ...
  Beyond this, to include lines, the \path{\cmidrule(l{<length>}r{<length>}){start_cell-end_cell}} from the \path{booktabs} package is used.
  ...
  Note that both line trim specifications do not need to be specified, in-fact one or none can be provide if the line does not need to be trimmed, \textit{e.g.}, \path{\cmidrule{1-5}} or \path{\cmidrule(r{3em}){2-3}}
  \begin{table}[H]
  ...
  To enter and exit math mode one just needs to use the \path{\(<Equation>\)} symbols around an equation. While there also exists \path{$<Equation>$} to add math, it is not recommended due to potential compatibility issues. Additionally, this, \path{\(<Equation>\)}, method is capable of being redefined to add further customization. 
  An example of using math mode to get an inline equation is by using the following command:
  
  {\begin{Center}\path{\(\vec{F_{d}}=\frac{1}{2}\ A\ C_{d}\ \vec{V}^{2}\)}\end{Center}}
  ...
  \begin{equation*}
    E = m\ c^2
  \end{equation*}
  The second will produce an equation with a reference. For this, there are two many ways of creating the reference, the first one, see \Cref{eq:Eq}, creates a numbered reference, the other one, see \Cref{eq:customTag}, creates a reference with a `tag'. The difference between the two is just the inclusion of a \path{\tag{<text>}} command that will replace the regular number with \path{<text>}.
